...
`SUPABASE_SECRET_KEY`는 GitHub에 올리거나 브라우저에서 사용하는 `NEXT_PUBLIC_` 변수로 만들면 안 됩니다.
...
배포 환경에도 같은 `SUPABASE_URL`과 `SUPABASE_SECRET_KEY`를 서버 환경변수로 등록한 뒤 새 버전을 배포합니다. 연결 전에는 현재 운영 주소를 교체하지 않습니다.
