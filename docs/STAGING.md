...
- 실제 통합 테스트는 런타임 호환 설정과 별개로 `tests/helpers/integration-target-guard.mjs`의 고정 Staging allowlist를 통과해야 합니다. APP_ENV 누락/production, 다른 URL, 운영 차단 목록 누락은 Worker 로딩·클라이언트 생성 전에 실패합니다. 테스트 프로젝트 교체는 코드 리뷰 대상입니다.
- 테스트가 비활성화되면 실제 통합 3건은 SKIPPED이며 성공으로 계산하지 않습니다. `.env.local`을 자동으로 안전한 연결이라고 가정하지 않습니다.
- 이슈별 로컬 검증 후 관련 변경을 작은 묶음으로 스테이징에 배포합니다. 묶음마다 커밋·TC·결과를 기록하고, 사용자의 승인 전 GitHub merge 및 운영 배포를 하지 않습니다.
