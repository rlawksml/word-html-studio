...
- 공개 방문자 화면 회귀: **PASS**
- 실제 작업자 기간 입력 브라우저 시나리오: **PENDING** — 작업자 암호 사용 승인 및 Staging 암호 정상화 뒤 수행
...
1. Staging 작업자 암호 정상화 및 사용 승인 후 `기간 입력 → 적용 → 임시 저장 → 재진입 → 미리보기 → HTML` 시나리오를 검증한다.
2. 같은 최종 v1.1 시나리오에서 이슈 #42의 `달력 월 이동만으로 날짜가 추가되지 않음`도 다시 확인한다.
3. 두 수동 검증이 끝나기 전에는 PR을 최종 승인하거나 Production에 배포하지 않는다.
