...
HTML 편집자의 통합 작업 ZIP에는 브라우저 미리보기용 `.html`, 외부 게시판 HTML 편집기에 그대로 복사·붙여넣을 본문 마크업 `.txt`, 원본 사진과 사진 배치 안내가 함께 들어갑니다.

사진은 한 장당 20MB까지 받습니다. 브라우저는 모바일 미리보기를 만든 뒤 짧게 유효한 업로드 주소를 발급받아 원본과 미리보기를 Supabase Storage에 직접 전송합니다. 큰 파일이 Next.js 서버 요청 크기 제한에 걸려 `413` 오류가 나는 것을 피하면서도 Storage 비밀키와 원본 사진은 공개하지 않습니다. Windows에서 MIME 정보가 비어 있는 파일도 확장자로 판별하고, 브라우저가 직접 표시하지 못하는 HEIC/HEIF는 사진을 고를 때만 전용 변환기를 지연 로드해 JPEG 미리보기를 만듭니다.

파일을 드래그할 때 브라우저가 `drop`과 파일 입력 `change`를 연달아 발생시키더라도 한 번에 하나의 업로드만 진행합니다. 업로드 중에는 진행 안내를 표시하고 뒤로가기·임시 저장·입력 마무리가 사진의 DB 연결보다 먼저 실행되지 않게 합니다. 네트워크·Storage 5xx는 같은 전송을 짧게 재시도하고, 연결이 끊기거나 서명 주소가 무효해졌다면 새 주소를 발급받아 최대 3회 복구합니다. Supabase가 이미 저장된 예약 파일에 대해 HTTP `400` 안에 `409 Duplicate`를 반환하면 성공한 업로드로 이어서 처리합니다. 최종 실패 시 예약된 원본·미리보기를 정리하며, 사진 삭제는 DB에서 참조가 사라진 뒤 Storage 객체를 제거해 저장 실패가 깨진 이미지로 이어지지 않게 합니다.
...
v1.0.1부터 Submission 저장은 클라이언트가 아는 필드만으로 JSON 전체를 덮어쓰지 않습니다. 같은 ID의 기존 소식·추가 항목·링크·사진에 미래 버전 필드가 있으면 서버의 기존 값을 보존하고, 사용자가 수정한 현재 필드와 배열 순서만 반영합니다. 사용자가 삭제한 항목은 되살리지 않습니다. v1.1의 기간 일정은 구버전이 덮어쓰지 않는 별도 `news_schedule_ranges` 테이블을 사용하고, 본문과 한 트랜잭션으로 저장합니다. 기간을 지우면 행을 삭제하지 않고 비활성화해 롤백·감사 시 복구할 수 있습니다. `app_schema_versions`와 `/api/version`으로 앱·DB 조합을 확인합니다. 자세한 원칙은 [DB migration·앱 롤백 호환 정책](docs/DB_MIGRATION_POLICY.md)에 있습니다.
...
6. [use-workspace-persistence.ts](apps/web/hooks/use-workspace-persistence.ts): 레코드 변경 감지, 저장 직렬화와 충돌 처리
...
- [이슈 #62 운영 의존성 보안 업데이트 Staging 검증 보고서](docs/test-reports/2026-09-08-05a8a5c-issue-62-staging.md)
- [이슈 #64 Cloudflare·Vite 개발 도구 보안 업데이트 Staging 검증 보고서](docs/test-reports/2026-09-08-1737a98-issue-64-staging.md)
- [이슈 #66 vinext 이미지 파서 보안 업데이트 Staging 검증 보고서](docs/test-reports/2026-09-08-12b8647-issue-66-staging.md)
