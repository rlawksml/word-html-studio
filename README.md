...
- 방문자: 지관서가 동네책방 페이지 바로가기, 책방·소식 수 요약, 이전·다음 달 이동, 책방별 색상 일정과 툴팁을 갖춘 간결한 모바일 달력, 책방 이름·이벤트 제목 중심의 카드, 디바운스 검색
- 정보 입력자: 한 책방씩 작성, 책방 기본정보 관리, 여러 소식·날짜·사진, 드래그 순서 변경
- 저장: 1.2초 뒤 자동 임시 저장과 수동 임시 저장, 지난달 내용 복사
- 완료: 책방별 입력 완료, 수정 시 자동으로 작성 중 전환, 책방별 소식 제목이 담긴 월 전체 완료 내용 복사
...
방문자 헤더의 `소식 입력` 또는 `HTML 편집` 버튼을 먼저 선택한 뒤 해당 역할의 암호를 입력합니다. 현재 작업자 접근 코드는 프로토타입 확인용입니다.
