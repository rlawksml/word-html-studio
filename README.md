...
- 정보 입력자: `지관서가` 또는 `wlrhkstjrk`
- HTML 편집자: `지관서가2` 또는 `wlrhkstjrk2`
- 방문자: 코드 없이 바로 이용

한글·영문 자판 중 어느 상태로 입력해도 같은 역할로 접속됩니다. 작업자 상태는 `sessionStorage`에 보관되어 새로고침에는 유지되고, 탭을 닫거나 로그아웃하면 사라집니다.
