...
type BookstoreManagementProps = {
  ...
  saveBookstore: (bookstore: Bookstore) => Promise<Bookstore>;
  onBack: () => void;
  notify: (message: string) => void;
};

// 매월 반복해서 사용하는 책방 기본정보를 등록·수정합니다. 월별 소식은 이 컴포넌트에서 다루지 않습니다.
export function BookstoreManagement({ bookstores, saveBookstore, onBack, notify }: BookstoreManagementProps) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try {
      await saveBookstore({ ...form, sortOrder: editingId === null ? bookstores.length : form.sortOrder });
      edit();
    } catch (error) {
      notify(error instanceof Error ? error.message : "책방 정보를 저장하지 못했습니다.");
    } finally {
      setSaving(false);
    }
  };
  ...
  return <div className="bookstore-management">
    <div className="workspace-heading"><div><button className="back-button" onClick={onBack} disabled={saving}>← 뒤로가기</button><h1>책방 관리</h1><p>한 번 저장한 기본정보는 매월 소식 작성에 다시 사용됩니다.</p></div><button className="primary-button" onClick={() => edit()} disabled={saving}>＋ 새 책방</button></div>
    <div className="management-grid">
      <div className="management-list">{bookstores.map((bookstore) => <button className={editingId === bookstore.id ? "active" : ""} key={bookstore.id} onClick={() => edit(bookstore)} disabled={saving}><strong>{bookstore.name}</strong><span>{bookstore.region}</span><small>{bookstore.address || "주소 미등록"}</small></button>)}</div>
      <div className="management-form" aria-busy={saving || undefined} inert={saving || undefined}><h2>{editingId === null ? "새 책방 등록" : "책방 정보 수정"}</h2>
        ...
        <div className="form-actions"><button className="secondary-button" onClick={() => edit()} disabled={saving}>취소</button><button className="primary-button" onClick={() => void save()} disabled={saving}>{saving ? "공용 저장소에 저장 중..." : "저장"}</button></div>
      </div>
    </div>
  </div>;
}
