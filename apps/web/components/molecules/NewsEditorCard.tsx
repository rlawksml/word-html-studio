...
export function NewsEditorCard({ studio, news, index, total }: NewsEditorCardProps) {
  ...
  const uploadsLocked = imageUploadNewsId !== null;
  return <article className="news-editor-card" onDragOver={(event) => event.preventDefault()} onDrop={() => reorderNews(news.id)}>
    <div className="news-editor-head"><button
      type="button"
      className="drag-handle"
      draggable
      aria-label={`소식 ${index + 1} 순서 이동`}
      title="이 손잡이를 끌어서 소식 순서를 바꿉니다"
      onDragStart={(event) => {
        event.dataTransfer.effectAllowed = "move";
        event.dataTransfer.setData("text/plain", String(news.id));
        setDraggedNewsId(news.id);
      }}
      onDragEnd={() => setDraggedNewsId(null)}
    >⠿</button><div><small>소식 {index + 1}</small><h2>{news.title || "새 소식"}</h2></div><div className="reorder-buttons"><button type="button" onClick={() => moveNews(news.id, -1)} disabled={index === 0}>↑</button><button type="button" onClick={() => moveNews(news.id, 1)} disabled={index === total - 1}>↓</button><button type="button" className="danger" onClick={() => updateCurrent((submission) => ({ ...submission, news: submission.news.filter((item) => item.id !== news.id) }))} disabled={total === 1}>삭제</button></div></div>
    <div className="form-grid">
      ...
      <label className="wide"><span>사진 <em>선택 · 여러 장 가능</em></span><div className={`upload-box${uploading ? " is-uploading" : ""}`} aria-busy={uploading || undefined} onDragOver={(event) => { event.preventDefault(); event.stopPropagation(); }} onDrop={(event) => { event.preventDefault(); event.stopPropagation(); if (!uploadsLocked) void addImages(Array.from(event.dataTransfer.files), news.id); }}><input type="file" accept="image/*,.heic,.heif" multiple disabled={uploadsLocked} onChange={(event) => { void addImages(Array.from(event.target.files || []), news.id); event.target.value = ""; }} /><strong>{uploading ? "사진을 안전하게 저장하는 중..." : "＋ 사진 첨부하기"}</strong><small>{uploading ? "업로드와 소식 연결이 끝날 때까지 이 화면을 유지해 주세요." : "여러 사진을 끌어다 놓을 수 있습니다. 사진 한 장은 최대 20MB입니다."}</small></div></label>
    </div>

    {news.images.length > 0 && <div className="photo-editor-grid">{news.images.map((image, imageIndex) => <figure key={image.id} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.stopPropagation(); reorderImages(news.id, image.id); }}>
      <button type="button" className="photo-drag-handle" draggable aria-label={`사진 ${imageIndex + 1} 순서 이동`} title="이 번호를 끌어서 사진 순서를 바꿉니다" onDragStart={(event) => { event.stopPropagation(); event.dataTransfer.effectAllowed = "move"; event.dataTransfer.setData("text/plain", String(image.id)); setDraggedImageId(image.id); }} onDragEnd={() => setDraggedImageId(null)}>{imageIndex + 1}</button><img src={image.url} alt={image.caption || image.name} />
    ...
    <details className="optional-fields">
      ...
      <div className="form-grid optional-fields-grid">
        ...
        <label className="wide"><span>대표 신청 링크 <em>선택</em></span><input data-url-field-path={`news.${index}.applyUrl`} value={news.applyUrl} onChange={(event) => updateNews(news.id, "applyUrl", event.target.value)} onBlur={(event) => { const normalized = normalizeHttpUrlInput(event.target.value); if (normalized !== news.applyUrl) updateNews(news.id, "applyUrl", normalized); }} placeholder="https://example.com 또는 www.example.com" /></label>
      </div>
      ...
      <div className="repeatable-section"><div><strong>관련 링크</strong><small>신청 페이지, 소개 글, SNS 등 링크를 여러 개 넣을 수 있습니다.</small></div>{news.links.map((link, linkIndex) => <div className="repeatable-row" key={link.id}><input value={link.label} onChange={(event) => updateNewsValue(news.id, "links", link.id, "label", event.target.value)} placeholder="링크 이름" /><input data-url-field-path={`news.${index}.links.${linkIndex}.url`} value={link.url} onChange={(event) => updateNewsValue(news.id, "links", link.id, "url", event.target.value)} onBlur={(event) => { const normalized = normalizeHttpUrlInput(event.target.value); if (normalized !== link.url) updateNewsValue(news.id, "links", link.id, "url", normalized); }} placeholder="https://example.com 또는 www.example.com" /><button onClick={() => updateCurrent((submission) => ({ ...submission, news: submission.news.map((item) => item.id === news.id ? { ...item, links: item.links.filter((entry) => entry.id !== link.id) } : item) }))}>삭제</button></div>)}<button className="text-add-button" onClick={() => updateCurrent((submission) => ({ ...submission, news: submission.news.map((item) => item.id === news.id ? { ...item, links: [...item.links, makeLink()] } : item) }))}>＋ 링크 추가</button></div>
    </details>
  </article>;
}
